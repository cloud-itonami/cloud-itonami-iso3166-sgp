(ns marketentry.marketentryllm
  ...
  the SGP public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-sg-entity?=" (:has-sg-entity? e)
                        " uen-verified?=" (:uen-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-sg-entity? e))
                              (:has-sg-entity? e))
                          (or (not (:requires-uen? e))
                              (:uen-verified? e)))
                   0.9 0.3)}))
