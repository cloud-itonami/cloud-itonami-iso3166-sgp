    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(UEN/統一資格/GeBIZ登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- sg-entity-missing-violations
...
(defn- uen-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-uen? true`, INDEPENDENTLY check
  `:uen-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-uen? e))
                 (not (true? (:uen-verified? e))))
        [{:rule :uen-unverified
          :detail (str subject " はUEN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (sg-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (uen-unverified-violations request st)
