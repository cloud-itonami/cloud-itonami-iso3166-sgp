...
2. Register the client's intake: business type, target public function,
   prior filing history in Singapore if any.
3. Run the advisor in read-only mode against the GeBIZ (Government Electronic Business) portal for government tenders and quotations.
...
- named referral relationship with Singapore-licensed counsel or a
...
a working referral relationship with Singapore-licensed counsel or a
registered agent for whatever licensed representation the law of
Singapore requires for actual public-procurement filings.
